...
Este guia foi elaborado com base nos critérios de avaliação da **2ª Avaliação (Menção Parcial 2)** para ajudá-los a conduzir uma apresentação fluida, técnica e segura perante a banca, dividindo de forma clara as falas e demonstrações de cada integrante.
...
| **5. Defesa e Perguntas (Q&A)** | 2 min | **Ambos** | Responder às arguições técnicas. |
...
*   **O Problema de Negócio:** Investidores de criptoativos precisam de dados históricos longos (para análises de tendência) mas também dados recentes de alta resolução. Salvar tudo com alta frequência estouraria o armazenamento. A solução é coletar dados em tempo real e aplicar agregações de série temporal (*downsampling*).
*   **A Evolução dos Domínios (Feedback da Prova 1):** Explique que na P1 os domínios estavam acoplados às etapas do pipeline. Agora, foram organizados por **domínios de negócio/conhecimento**:
...
> **Sugestão de Fala para o João:**
> *“O BTC Data Pipeline foi projetado para solucionar um trade-off clássico em engenharia de dados: o custo de armazenamento contra a resolução recente dos dados. Para isso, estruturamos o pipeline sob uma arquitetura Medalhão. Além disso, corrigimos a modelagem de domínios em relação à etapa anterior. Agora, o projeto divide-se em quatro domínios de negócio bem delimitados, separando a inteligência de mercado cripto da lógica cambial e analítica temporal.”*
...
*   **A. O Banco de Dados (PostgreSQL):**
    *   *O que abrir:* Um cliente SQL (ex: DBeaver, pgAdmin) ou via linha de comando no container.
...
*   **B. A Orquestração (Apache Airflow):**
    ...
    *   *O que mostrar:*
        ...
        3.  DAG `btc_historical_backfill` (backfill manual). **Clique em "Trigger DAG" em tempo real** para demonstrar os containers rodando o `dbt run` com sucesso.

> **Sugestão de Fala para o João:**
> *“Vou demonstrar como os dados são organizados no banco de dados Postgres e como o fluxo é orquestrado no Airflow. No Postgres, temos isolamento completo por schemas: a camada bronze armazena os JSONs brutos da API, a camada silver unifica e limpa os dados, e a camada gold calcula as agregações analíticas de downsampling. No painel do Airflow, temos a DAG de Ingestão Contínua que roda a cada 5 minutos, e a DAG de Backfill Histórico, que vou disparar agora manualmente para demonstrar a carga em lote dos últimos 30 dias em tempo real.”*
...
*   **P: Por que usar dbt em vez de scripts Python/Pandas convencionais? (Defesa: João)**
    ...
    *   
> **Sugestão de Fala para o João:**
> *“O dbt centraliza o processamento SQL dentro da própria base de dados (ELT). Em vez de extrair os dados para a memória da aplicação com Pandas, processamos tudo no Postgres. Ele também gerencia DDLs automaticamente e nos dá testes integrados e linhagem de dados nativos, o que o torna um padrão muito mais profissional de mercado.”*
